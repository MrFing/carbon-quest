...
async def finalize_disconnect(session_id: str, player: int, token: int) -> None:
    ...
    if session.connected_players() == 0:
        logger.info(
            "Removing abandoned session %s after both players disconnected", session_id)
        session_manager.remove_session(session_id)
        return

    logger.info(
        "Player %s is still disconnected from session %s after grace period", player, session_id)
...
async def create_party() -> PartyResponse:
    ...
    logger.info("Created party %s for session %s",
                session.party_code, session.session_id)
...
async def get_party(code: str) -> PartyResponse:
    ...
    logger.info("Resolved party code %s to session %s",
    ...
    try:
        ...
        while True:
            ...
            if message_type == "ROLL_DICE":
                ...
                try:
                    state = session.game.make_choice(
                        player, payload.get("choice"))
                except ValueError as exc:
                    await safe_send(websocket, ErrorMessage(message=str(exc)).model_dump())
                    continue
        ...
        if current_session.get_socket(player) is websocket:
            ...
            logger.info(
                "Player %s disconnected from session %s, waiting for reconnect", player, session_id)
            asyncio.create_task(finalize_disconnect(session_id, player, token))
    except Exception:
        logger.exception(
            "Unexpected websocket error in session %s for player %s", session_id, player)
        current_session = session_manager.get_session_by_id(session_id)
    ...
    while True:
        ...
        try:
            async with httpx.AsyncClient() as client:
                await client.get("https://YOUR-RENDER-URL.onrender.com/health")
        except:
            pass
