...
SCALE = HEIGHT / 1080  # If you designed for 1080p, this is 2.0


def load_ui_image(path, original_width):
    img = pygame.image.load(path).convert_alpha()
    new_size = (int(original_width * SCALE), int(img.get_height()
                * (original_width * SCALE / img.get_width())))
...
def draw_glow(surface, rect, color, amount=70, radius=18):
...
class CarbonQuest:
    def __init__(self):
        ...
        self.policy_buttons = []
        self.roll_button = Button((1065, 660, 140, 38), "Roll Dice", BLUE)
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if self.state == "play":
                ...
                eco_area = pygame.Rect(910, 820, 170, 62)
                quick_area = pygame.Rect(1075, 820, 170, 62)
    ...
    def draw_card(self, surface):
        ...
        if self.selected_event:
            ...
            draw_text(surface, FONT_TINY, "Pick an action",
                      YELLOW, (rect.x + 16, rect.y + 122))
            return
        if not self.selected_card:
            draw_text(surface, FONT_H1, f"{self.current_player().name}'s Turn", self.current_player(
        ...
        eco_cost = card.eco_effect[2]
        quick_cost = card.quick_effect[2]
        ...
        draw_text(surface, FONT_H1, card.title,
                  ZONE_COLORS[card.zone], (rect.x + 16, rect.y + 8))
        eco_rect = pygame.Rect(rect.x + 16, rect.y + 60, 225, 75)
        quick_rect = pygame.Rect(rect.x + 256, rect.y + 60, 225, 75)
        pygame.draw.rect(surface, (16, 80, 45), eco_rect, border_radius=9)
        pygame.draw.rect(surface, (100, 25, 25), quick_rect, border_radius=9)
        draw_text(surface, FONT_SMALL, "ECO",
        ...
        draw_text(surface, FONT_TINY, f"Cost: {money(-eco_cost)}", WHITE,
                  (eco_rect.x + 12, eco_rect.y + 55))
        draw_text(surface, FONT_SMALL, "QUICK", (254, 202, 202),
        ...
        draw_text(surface, FONT_TINY, f"Cost: {money(-quick_cost)}", WHITE,
